...
"""
    ...
    return {"full_chords":clist, 
            ...
            "transposed_seqs_skip":transposed_seqs_skip,
            "transposed_seqs_noskip":transposed_seqs_noskip}
"""

def get_sequences_onefile(allpath):
    print(allpath)
...
def main():
    ...
    for epoch in range(TRAIN_ITER // EPOCH_ITER):
        ...
        unsupervised_chord_keys, unsupervised_chord_keys_onehot, unsupervised_chord_notes, unsupn0,unsupn1,unsupn2,unsupn3 = train2.train_epoch(
                        ...
                        for k in range(len(seq)):
                            ...
                            if val != None:
                                if isinstance(val, (list,tuple)):
                                    ...
                                    seq[k] = int(seq[k])
            with open("gens_t"+str(ecount)+".json",'w') as dumpfile:
                    ...
                    for k in range(len(seq)):
                        ...
                        if val != None:
                            if isinstance(val, (list,tuple)):
                                ...
                                seq[k] = int(seq[k])
            with open("gens_g"+str(ecount)+".json",'w') as dumpfile:
                json.dump(generations,dumpfile)
            with open("gens_5g"+str(ecount)+".json",'w') as dumpfile:
                json.dump(generations5,dumpfile)
            with open("gens_7g"+str(ecount)+".json",'w') as dumpfile:
                json.dump(generations7,dumpfile)

            print("------SAVING CURRENT MODEL------")
            savefile = 'savedir_new/'+parsename+'_model' + str(saveNum)
