...
G_STEPS = 7  # how many times to train_bit the generator each round
D_STEPS = 1  # how many times to train_bit the discriminator per generator steps
...
def main():
    ...
    for epoch in listofepochs:
        ...
        unsupervised_chord_keys, unsupervised_chord_keys_onehot, unsupervised_chord_notes, unsupervised_sps, unsupervised_sps_dur = train_bit.train_epoch(
        ...
        if not startUnsup and latest_d_loss != None and latest_d_loss < 0.5:
            ...
            skipD = True
        if latest_g_loss != None and latest_g_loss < G_LOSS_BOUNDARY and not startUnsup:
            startUnsup = True
        if startUnsup:
            skipG = False
            skipD = False
